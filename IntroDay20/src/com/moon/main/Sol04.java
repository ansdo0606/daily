...
public class Sol04 {
	public static String solution(String polynomial) {
        ...
        for (String s : arr) {
            if (s.contains("x")) {
                if (s.equals("x")) {
                    ...
                    int len = s.length();
                    int coefficient = len == 1 ? 1 : Integer.parseInt(s.substring(0, len - 1));
                    xNum += coefficient;
                }
            } else {
        ...
        if (xNum == 0) {
            ...
            answer.append(xNum != 1 ? xNum + "x" : "x");
        } else {
            answer.append(xNum != 1 ? xNum + "x" : "x").append(num > 0 ? " + " : " - ").append(Math.abs(num));
        }
	 ...
	 public static void main(String[] args) {
	        ...
	        System.out.println(result1); // "4x + 7" 출력

	        String polynomial2 = "x + x + x";
	        String result2 = solution(polynomial2);
	        System.out.println(result2); // "3x" 출력
	    }
}
