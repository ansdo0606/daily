 ...
 * 로그인 성공?
	...
	정보가 담긴 2차원 배열 db가 주어질 때, 다음과 같이 로그인 성공, 실패에 따른 메시지를 return하도록 solution 
	함수를 완성해주세요.
...
public class Sol04 {
	        ...
	        for (String[] member : db) {
	            ...
	            if (id.equals(memberId)) {
	                if (pw.equals(memberPw)) {
	                    return "login"; // 아이디와 패스워드 모두 일치
	                } else {
	                    return "wrong pw"; // 아이디는 일치하지만 패스워드 불일치
	                }
	            }
	        }
	        
	        return "fail"; // 아이디가 일치하는 회원 정보가 없음
	    }
	 public static void main(String[] args) {
	        ...
	        System.out.println(result1); // Output: "login"
